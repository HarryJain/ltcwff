# -*- coding: utf-8 -*-
"""
Created on Wed Jul 28 17:48:42 2021
...
latest = list(pd.read_sql('SELECT Date FROM games', conn)['Date'])[-1]
...
# Combine the new game rows with the pre-existing table and write the combined DataFrame to the database
old_df = pd.read_sql('SELECT * FROM games', conn).set_index('game_id')
new_df = pd.concat([old_df, big_df])
print(new_df)
new_df.to_sql('games', conn, if_exists = 'replace')
