# -*- coding: utf-8 -*-
"""
Created on Sat Feb 13 21:58:01 2021
...
def get_all_games(month, day, year):
    ...
    for table in tables:
        #row_1 = {'Date': f'{year}{month:02}{day:02}', 'Home Team': table.iloc[0, 0], 'Away Team': table.iloc[1, 0], 'Home 1': table.iloc[0, 1], 'Away 1': table.iloc[1, 1], 'Home 2': table.iloc[0, 2], 'Away 2': table.iloc[1, 2], 'Home 3': table.iloc[0, 3], 'Away 3': table.iloc[1, 3], 'Home 4': table.iloc[0, 4], 'Away 4': table.iloc[1, 4], 'Home T': int(table.iloc[0, 1]) + int(table.iloc[0, 2]) + int(table.iloc[0, 3]) + int(table.iloc[0, 4]), 'Away T': int(table.iloc[1, 1]) + int(table.iloc[1, 2]) + int(table.iloc[1, 3]) + int(table.iloc[1, 4])}
